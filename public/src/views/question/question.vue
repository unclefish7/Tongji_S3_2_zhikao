...
<!--      <el-table-column prop="id" label="题号"></el-table-column>-->
      ...
      <el-table-column label="题目">
        <template slot-scope="scope">
          <!-- 使用v-html指令渲染富文本内容 -->
          <div v-html="renderRichText(scope.row.richTextContent)"></div>
        </template>
      </el-table-column>
      <el-table-column prop="score" label="分数"></el-table-column>
...
export default {
  ...
  methods: {
    renderRichText(htmlContent) {
      // 使用正则匹配并渲染 LaTeX 公式
      const formulaRegex = /<span[^>]*data-value="([^"]+)"/g;

      // 替换 LaTeX 公式
      const formattedHtml = htmlContent.replace(formulaRegex, (match, latex) => {
        // 使用 KaTeX 渲染公式
        const renderedLatex = katex.renderToString(latex, { throwOnError: false });
        // 替换 LaTeX 公式为已渲染的 HTML 代码
        return match.replace(latex, renderedLatex);
      });
